...
if ! [[ -f 'Server-Files-0.2.61.zip' ]]; then
	...
	curl -Lo 'Server-Files-0.2.61.zip' 'https://edge.forgecdn.net/files/5458/456/Server-Files-0.2.61.zip' || exit 9
	unzip -u -o 'Server-Files-0.2.61.zip' -d /data
 ...
 FILE=mohist-${FORGE_VERSION}-installer.jar
if [ -f "$FILE" ]; then
    if [[ -n "$JVM_OPTS" ]]; then
